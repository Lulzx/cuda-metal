...
stands at **4 of 11 compiling** (`mesh`, `hashgrid`, `scan`, `volume`).
...
| File | Blocker | Owner |
|---|---|---|
| `warp.cu` | `nvPTXCompiler.h` and `nvrtc.h`: Warp compiles user kernels and its conditional-graph helpers through NVRTC at runtime. CuMetal needs an NVRTC shim whose `nvrtcCompileProgram` runs `cumetalc` and whose `nvrtcGetCUBIN` returns the metallib bytes (`cuModuleLoadDataEx` already accepts them), plus a pass-through `nvPTXCompiler` (PTX text is loadable directly). This is the real remaining Phase 1 item. | CuMetal |
...
| `volume_builder.cu` | placement `new` in device code without `#include <new>`; nvcc includes it implicitly, Clang does not | Warp (one-line include) |

Until `warp.cu` compiles, the practical route for Phase 1's gate is the
generated-kernel path proven above plus an NVRTC shim; nothing else in Warp's
Python layer needs to change.
