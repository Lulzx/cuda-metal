...
[Known-gaps index](known-gaps.md) · [Compiler gaps](known-gaps/compiler.md) ·
[Compiler architecture](compiler-architecture.md)

The typed PTX importer does not guess. It accepts a construct when it can
prove a bounded fact about it, and refuses otherwise. This page records what
each proof establishes, what it deliberately does not, and the budgets that
bound it. It explains *why* a given kernel is refused. The
[known-gaps index](known-gaps.md) answers the shorter question of *what* is
refused.

Each paragraph is a contract. Changing the analysis it describes requires
...
bit-count and bit-reversal builtins are also accepted in trap-capable helper graphs.
