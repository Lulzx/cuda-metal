...
- The typed shared-IR migration gate is not closed. With CUDA Clang 21-23, the
  reviewed 25-file production-metallib matrix is 25/25 for direct `.cu` through
...
- The exact 23-project in-tree corpus now passes 23/23 through typed PTX and
  23/23 through direct native AOT on Apple M4 Pro with workload
...
| P1 | Canonical compiler path | Expand residual generic-pointer/device-call/CFG combinations beyond the now-matched 25-file compile corpus. Direct scalar-return helpers with pointer arguments, loops, merges, early exits, and transitive `printf` state are proven; aggregate/multi-result/indirect call forms and broader barrier CFG remain open. Preserve native-AOT symbol metadata and cache identity/corruption rejection throughout the migration. | `conformance_compiler_backend_matrix_versions` plus numerical Apple-GPU tests show typed IR at least matches the legacy pass count across CUDA Clang 21-23; `functional_cuda_projects_device_calls_typed_ptx` and the exact typed-PTX/native-AOT corpora prove the direct-call subset without workload specialization; `functional_cumetalc_link_executable` proves an embedded native module with no `__cudaRegister*` dependency, no first-launch PTX JIT, and no registration-JIT cache; `functional_cumetalc_native_aot_symbols` proves host copies and persistent GPU writes for constant/device globals; `unit_module_cache` remains green. Only then change remaining backend defaults. |
