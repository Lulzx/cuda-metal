...
| Cooperative groups | No hits for `cooperative_groups`, `grid_group`, or `this_grid()`. `particleSystemHFMidPhaseCG.cu` uses “CG” in a contact-generation name, not the cooperative-groups API. The iterative TGS path loops on the host and submits partition solve, velocity averaging, static propagation, articulation, conclude, and writeback as ordered kernel launches. | CuMetal supports resident cooperative grids capped at one block per reported GPU core and rejects oversubscribed grids. PhysX already uses the multi-dispatch phase structure and does not depend on this subset. | **Fine**. |
| `grid_group::sync` | No hits. The multiblock solver name describes work partitioning, not an in-kernel grid barrier. | Resident grids use the bounded sense-reversing barrier; arbitrary oversubscribed grids remain unsupported. | **Fine**. |
| Dynamic parallelism | No `<<<...>>>` launch syntax or device-launch API hits in the GPU sources. Host code launches through `cuLaunchKernel`. | CuMetal has a bounded host-drained device-launch queue, but PhysX does not require it. | **Fine** because absent. |
| Textures/surfaces | Three active `tex3D<float>` calls in `gpunarrowphase/src/CUDA/sdfCollision.cuh` at lines 118, 134, and 147. Two `tex1Dfetch` lines in `bvh.cuh` are commented. `CudaKernelWrangler.cpp` defines an empty `__cudaRegisterTexture` compatibility function. Host GPU code also uses Driver API array/texture-object calls for SDF assets. No active surface instruction was found. | CuMetal's Runtime API source-level descriptor helpers cover a tested `tex3D` subset, but the equivalent PhysX Driver API object/binding path and broad SDF semantics remain unverified. Direct PTX dimension queries alone are supported; sampling instructions remain unsupported. | **Blocker pending an end-to-end SDF test**. **Fine** for the sphere-plane GRB target, which does not select this code. |
| `cudaLaunchCooperativeKernel` | No Runtime or Driver API cooperative-launch hit in PhysX GPU code. | CuMetal supports bounded resident-grid cooperative launches and rejects oversubscribed grids. | **Fine** because absent. |
...
| `double` | Active device double precision occurs in `gpusimulationcontroller/src/CUDA/bvh.cuh:356-363` (six robust geometric products) and `gpunarrowphase/src/CUDA/femClothPrimitives.cu:3578` (one plane distance). Other hits are comments or host-only code such as `PxgFEMCloth.cpp:632`. | `fast48`, `wide48`, and `ieee64` preserve binary64 memory ABI, but their operation coverage and performance differ; no end-to-end oracle covers these PhysX paths. | **Needs validation and an explicit FP64 mode choice** for BVH construction and FEM cloth. **Fine** for the primitive sphere-plane target because those units/paths are excluded. |
