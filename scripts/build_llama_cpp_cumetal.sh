...
for libdir in lib lib64; do
    ...
    # libcumetal's install name is @rpath/libcumetal.dylib, so anything that
    # links -lcuda and adds this directory to its rpath needs to find the
    # library under its own name too.
    ln -sf "${CUMETAL_ACTIVE_BUILD_DIR}/libcumetal.dylib" "${FAKE_CUDA}/${libdir}/libcumetal.dylib" 2>/dev/null || true
