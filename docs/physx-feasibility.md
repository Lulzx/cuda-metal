...
| Cooperative groups | No hits for `cooperative_groups`, `grid_group`, or `this_grid()`. `particleSystemHFMidPhaseCG.cu` uses “CG” in a contact-generation name, not the cooperative-groups API. The iterative TGS path loops on the host and submits partition solve, velocity averaging, static propagation, articulation, conclude, and writeback as ordered kernel launches. | CuMetal rejects unsafe multi-block cooperative launches. PhysX already uses the multi-dispatch phase structure that a general kernel-fission implementation would need to synthesize. | **Fine**. |
| `grid_group::sync` | No hits. The multiblock solver name describes work partitioning, not an in-kernel grid barrier. | Single-block cooperative sync reduces to a threadgroup barrier; multi-block launch is rejected. Typed-IR kernel fission remains future work for unrelated cooperative CUDA applications. | **Fine**. |
...
| `cudaLaunchCooperativeKernel` | No Runtime or Driver API cooperative-launch hit in PhysX GPU code. | CuMetal supports a single threadgroup and rejects multi-block launch with not-supported. | **Fine** because absent. |
