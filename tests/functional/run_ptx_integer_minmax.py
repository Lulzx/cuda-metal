#!/usr/bin/env python3
"""Numerical Apple-GPU regression for 64-bit signed and unsigned min/max."""
import ctypes as c
import os
...
PTX = (Path(__file__).parent / 'reference/ptx_integer_minmax.ptx').read_text()
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='cumetal-integer-minmax-') as work:
            ptx, msl = Path(work) / 'test.ptx', Path(work) / 'test.metal'
            ptx.write_text(PTX)
            subprocess.run([str(build / 'cumetalc'), str(ptx), '--backend=cumetal-ir',
                            '--ptx-strict', '--entry', 'integer_minmax', '--emit=msl', '-o', str(msl)], check=True)
            api('cuModuleLoad', [c.POINTER(ptr), c.c_char_p], c.byref(module), os.fsencode(msl))
            api('cuModuleGetFunction', [c.POINTER(ptr), ptr, c.c_char_p], c.byref(function), module, b'integer_minmax')
            ...
            print('NUMERICAL_PASS integer_minmax: 12 boundary inputs; unsigned/signed min/max and immediate operands, guards')
    finally:
