...
public:
    ...
    // Reinterpret float/integer storage, never a numeric conversion. A wider
    // integer container contributes only the instruction's low source bits.
    Operand bit_container(Operand input, const Type& expected) {
        if (input.type.kind == TypeKind::kInteger && expected.kind == TypeKind::kFloat &&
            input.type.bit_width > expected.bit_width)
            input = low_integer_bits(std::move(input), Type::integer(expected.bit_width));
        if (input.type.kind == TypeKind::kFloat && expected.kind == TypeKind::kInteger &&
            input.type.bit_width > expected.bit_width) {
            const auto bits = Type::integer(input.type.bit_width);
            input = emit(OpCode::kConvert, bits, {std::move(input)}, {{"bitcast", "true"}});
            return low_integer_bits(std::move(input), expected);
        }
