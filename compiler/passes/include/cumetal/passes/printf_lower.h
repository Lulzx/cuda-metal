...
struct PrintfLoweredCall {
    ...
    // CUDA specifies -1 for a statically null format pointer. Keep the call in the
    // printf call graph so hidden ABI propagation remains intact, but emit no
    // ring-buffer record for it.
