...
std::uint32_t ptx_param_size(const cumetal::ptx::Parameter &param) {
  ...
  if (open != std::string::npos && close == param.name.size() - 1 &&
      close > open + 1) {
    const std::string count_text =
        param.name.substr(open + 1, close - open - 1);
    char *end = nullptr;
...
std::string build_ptx_abi_sidecar(std::string_view ptx_source,
  ...
  for (const auto &entry : parsed.module.entries) {
    ...
    text += "shared " +
            std::to_string(cumetal::ptx::compute_static_shared_bytes(
                ptx_source, entry.name)) +
            "\n";
    for (const auto &param : entry.params) {
      text += param.is_pointer
                  ? "arg buffer 8\n"
...
void print_usage(const char *argv0) {
  std::cerr
      << "Usage: " << argv0
      << " <file.cu> -o <executable>            # compile and link a CUDA "
         "program\n"
         "       "
      << argv0
      << " <file.cu> -o <file.metallib>         # compile device code only\n\n"
         "Full options: "
      << argv0
      << " [--input] <file.{metal,cu,ptx,ll,air,bc}> [--output|-o "
         "<file.metallib>]"
...
std::filesystem::path make_temp_path(const std::string &extension_with_dot) {
  ...
  return std::filesystem::temp_directory_path() /
         ("cumetalc-ptx-" + std::to_string(pid) + "-" + std::to_string(now) +
          extension_with_dot);
}
...
bool command_exists(const std::string &name) {
  const CommandResult result =
      run_command_capture("command -v " + name + " >/dev/null 2>&1; echo $?");
...
bool xcrun_tool_exists(const std::string &tool_name) {
  const CommandResult result = run_command_capture(
      "xcrun --find " + quote_shell(tool_name) + " >/dev/null 2>&1; echo $?");
  if (!result.started || result.exit_code != 0 || result.output.empty()) {
    return false;
  }
  return result.output[0] == '0';
...
// Where cumetalc finds the pieces it needs to build a complete executable. The
// driver has to work both from the build tree and from `cmake --install`
// output, so nothing here may assume CUMETAL_SOURCE_DIR exists on disk.
...
// Absolute path of this cumetalc binary, used to locate sibling resources in an
// installed tree.
std::filesystem::path executable_path(const char *argv0) {
  ...
  if (argv0 != nullptr && argv0[0] != '\0') {
    ...
    const CommandResult found = run_command_capture(
        "command -v " + quote_shell(argv0) + " 2>/dev/null");
...
bool layout_from_root(const std::filesystem::path &include_dir,
                      ...
                      ResourceLayout *out) {
  if (!std::filesystem::exists(include_dir / "cuda_runtime.h"))
    return false;
  if (!std::filesystem::exists(lib_dir / "libcumetal.dylib"))
    return false;
...
ResourceLayout resolve_resources(const char *argv0) {
  ...
  // 1. Explicit override wins, so a relocated or unusual install is always
  // recoverable.
  if (const char *root = std::getenv("CUMETAL_ROOT");
      root != nullptr && root[0] != '\0') {
    const std::filesystem::path prefix(root);
    if (layout_from_root(prefix / "include", prefix / "lib",
  ...
  if (!self.empty()) {
    ...
    // 2. Installed layout: <prefix>/bin/cumetalc alongside
    // <prefix>/{include,lib,libexec}.
    ...
    // 3. Build tree: cumetalc sits next to libcumetal.dylib and
    // cuda_toolchain/, with the
    ...
    if (layout_from_root(source_api, bin_dir, bin_dir / "cuda_toolchain",
                         &layout)) {
      return layout;
    }
  }
...
// Homebrew LLVM defaults to PTX 4.2, which rejects newer -march values
// outright. Use Clang's CUDA-specific option so the PTX feature reaches only
// the device compilation; forwarding a raw -target-feature also sends it to the
// Apple arm64 host compilation and produces noisy warnings. Mirrors
// cumetal_cuda_ptx_feature_flags() in scripts/cumetal_cuda_flags.sh; keep the
// two in step.
std::string ptx_feature_flags_for_arch(const std::string &arch) {
  ...
  if (arch == "sm_80" || arch == "sm_86" || arch == "sm_89" ||
      arch.rfind("sm_90", 0) == 0) {
    return feature("ptx70");
  }
  if (arch == "sm_75" || arch == "sm_78")
    return feature("ptx63");
  if (arch == "sm_70" || arch == "sm_72")
    return feature("ptx60");
  if (arch == "sm_61")
    return feature("ptx50");
  return {};
}
...
std::filesystem::path find_cuda_clang(
  ...
  const CommandResult found =
      run_command_capture("command -v clang++ 2>/dev/null");
  if (found.started && found.exit_code == 0 && !found.output.empty()) {
    std::string path = found.output;
    while (!path.empty() &&
           std::isspace(static_cast<unsigned char>(path.back())) != 0) {
...
std::string ptx_feature_for_arch(std::string_view arch) {
  if (arch == "sm_90" || arch == "sm_89" || arch == "sm_86" ||
      arch == "sm_80") {
    return "+ptx70";
  }
  if (arch == "sm_78" || arch == "sm_75")
    return "+ptx63";
  if (arch == "sm_72" || arch == "sm_70")
    return "+ptx60";
  if (arch == "sm_61")
    return "+ptx50";
  return {};
...
std::string extension_for_stage(EmitStage stage) {
  switch (stage) {
  case EmitStage::kLlvm:
    return ".ll";
  case EmitStage::kCumetalIr:
    return ".cmir";
  case EmitStage::kMetalIr:
    return ".metalir";
  case EmitStage::kMsl:
    return ".metal";
  case EmitStage::kMetallib:
    return ".metallib";
  }
  return ".metallib";
}

bool write_text_output(const std::filesystem::path &output,
...
bool emit_inspection_stage(const cumetal::metal::PtxToMslResult &compiled,
  ...
  if (stage == EmitStage::kCumetalIr) {
    ...
    if (error != nullptr)
      *error = "requested stage is not a textual CuMetal output";
    return false;
  }
...
// Compile a complete CUDA translation unit -- host code and `<<<>>>` launches
// included -- into a runnable executable linked against libcumetal. This is
// spec.md's Phase 2/3 exit criterion.
//
// Clang does the whole job: it compiles the host side to the standard CUDA
// registration ABI
// (__cudaRegisterFatBinary / __cudaPushCallConfiguration / cudaLaunchKernel),
// and drives the device side through the in-tree ptxas and fatbinary shims,
// which pass PTX through into a fatbinary envelope rather than assembling SASS.
// libcumetal's registration path then JITs that PTX to a metallib on first
// launch. So the executable carries its own device code and needs no metallib
// path at runtime -- which is exactly what the old two-file sample flow could
// not do.
int run_executable_driver(const ExecutableDriverOptions &options,
                          const char *argv0) {
  const std::filesystem::path compiler = find_cuda_clang(options.cuda_clang);
  if (compiler.empty() || !std::filesystem::exists(compiler)) {
    std::cerr << "cumetalc failed: CUDA-capable clang++ not found; install "
                 "Homebrew LLVM or "
  ...
  if (!layout.ok) {
    std::cerr << "cumetalc failed: could not locate the CuMetal headers and "
                 ...
                 "  Set CUMETAL_ROOT to the install prefix, or run cumetalc "
                 "from the build "
                 "directory.\n";
    return 1;
  }
  if (!std::filesystem::exists(layout.toolchain_dir / "fatbinary") ||
    ...
    std::cerr << "cumetalc failed: the ptxas/fatbinary shims are missing from "
              ...
              << ".\n"
                 "  Clang needs them on PATH to emit device code for a linked "
                 "executable.\n";
    return 1;
  }
  ...
  // Clang execs the shims as subprocesses, so they must be found via PATH.
  // Prepend rather than replace so a user-provided toolchain earlier in PATH
  // still loses to ours deliberately.
  const char *existing_path = std::getenv("PATH");
  const std::string combined_path =
      layout.toolchain_dir.string() + ":" +
      (existing_path != nullptr ? std::string(existing_path)
                                : std::string("/usr/bin:/bin"));
  const std::string path_prefix = "PATH=" + quote_shell(combined_path) + " ";

  std::string compile =
      path_prefix + quote_shell(compiler.string()) +
      " -x cuda -std=c++17 -O2"
      " --cuda-gpu-arch=" +
  ...
  if (!compile_result.started || compile_result.exit_code != 0 ||
    ...
    std::cerr << "cumetalc failed: CUDA compilation of " << options.input
              << " failed\n";
    return 1;
  }
  ...
  const std::string link =
      quote_shell(compiler.string()) + " " + quote_shell(object_file.string()) +
      " -L " + quote_shell(layout.lib_dir.string()) + " -lcumetal -Wl,-rpath," +
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--input") {
      ...
      if (arg == "--emit") {
        if (i + 1 >= argc) {
          std::cerr << "--emit expects llvm, cumetal-ir, metal-ir, msl, "
                       "metallib, or exe\n";
          return 2;
        }
        value = argv[++i];
      ...
      if (value == "exe") {
        ...
        emit_stage = EmitStage::kLlvm;
      else if (value == "cumetal-ir")
        emit_stage = EmitStage::kCumetalIr;
      else if (value == "metal-ir")
        emit_stage = EmitStage::kMetalIr;
      else if (value == "msl")
        emit_stage = EmitStage::kMsl;
      else if (value == "metallib")
        emit_stage = EmitStage::kMetallib;
      else {
      ...
      if (cuda_inline_threshold.empty() ||
          cuda_inline_threshold.find_first_not_of("0123456789") !=
              std::string::npos) {
        std::cerr << "--cuda-inline-threshold expects a non-negative integer\n";
        return 2;
      }
  ...
  // Pick the backend that actually works for this input rather than one global
  // default. The two are complementary, not ranked, and the split follows the
  // frontend feeding them. Measured over the 19-file in-tree .cu corpus (tests/
  // + samples/, see docs/compiler-architecture.md):
  //
  ...
  // docs/known-gaps.md and lowers nothing in this corpus, so typed CuMetal IR
  // is strictly better there. Through the PTX frontend the ordering reverses
  // and defaulting to typed IR would regress the path llm.c, llama.cpp, and
  // PhysX all depend on. --backend overrides.
  if (!backend_set_explicitly && lower_ext(options.input) == ".cu" &&
      !cuda_device_frontend) {
    backend = BackendKind::kCumetalIr;
  }

  // `cumetalc foo.cu -o foo` builds an executable. Infer that from the shape of
  // the request -- a .cu input, the default (metallib) emit stage, and an -o
  // that does not name a .metallib -- so the nvcc-style invocation works
  // without a special flag. --link/--no-link override.
  ...
  if (link_executable) {
    if (lower_ext(options.input) != ".cu") {
      std::cerr << "cumetalc failed: --link requires a .cu input (got "
                << options.input << ")\n";
      return 2;
    }
  ...
  if (input_ext == ".cu" && cuda_device_frontend) {
    ...
    if (!cuda_inline_threshold.empty()) {
      command +=
          " -fgpu-inline-threshold=" + quote_shell(cuda_inline_threshold) +
          " -mllvm -inline-all-viable-calls";
    }
    if (std::filesystem::exists(runtime_api_dir) &&
        std::filesystem::is_directory(runtime_api_dir)) {
      command += " -I " + quote_shell(runtime_api_dir.string()) + " -include " +
                 quote_shell("cuda_runtime.h");
    }
  ...
  if (input_ext == ".ptx") {
    std::string io_error;
    const std::vector<std::uint8_t> ptx_bytes =
    ...
    const std::string ptx_source(
        reinterpret_cast<const char *>(ptx_bytes.data()), ptx_bytes.size());
    ...
    if (emit_stage == EmitStage::kLlvm) {
      ...
      if (!lowered.ok || !write_text_output(options.output, lowered.llvm_ir,
    ...
    if (backend == BackendKind::kCumetalIr) {
      ...
      if (!write_text_output(temp_stage_file, compiled.source, true,
                             &io_error)) {
      ...
      if (emit_stage == EmitStage::kCumetalIr ||
        ...
        std::cerr << "cumetalc failed: --emit="
                  << (emit_stage == EmitStage::kCumetalIr ? "cumetal-ir"
                                                          : "metal-ir")
      ...
      const auto lowered_metal = cumetal::ptx::lower_ptx_to_metal_source(
      ...
      if (lowered_metal.matched && !lowered_metal.metal_source.empty()) {
        ...
        if (!write_text_output(temp_stage_file, lowered_metal.metal_source,
                               true, &io_error)) {
          std::cerr
              << "cumetalc failed: failed to write temporary Metal source: "
              << io_error << "\n";
          return 1;
        }
        options.input = temp_stage_file;
        ...
        if (!write_text_output(temp_stage_file, lowered.llvm_ir, true,
                               &io_error)) {
    ...
    if (backend == BackendKind::kCumetalIr) {
      ...
      const auto compiled = cumetal::metal::compile_nvvm_to_msl(
          llvm_ir, options.input.string(), ptx_entry_name);
      if (!compiled.ok) {
        std::cerr << "cumetalc failed: " << compiled.error << "\n";
        return 1;
      }
      if (emit_stage == EmitStage::kLlvm) {
        if (!write_text_output(options.output, llvm_ir, options.overwrite,
                               &io_error)) {
          std::cerr << "cumetalc failed: " << io_error << "\n";
          return 1;
        }
    ...
    if (backend == BackendKind::kCumetalIr || emit_stage == EmitStage::kLlvm) {
      ...
      if (clang.empty()) {
        std::cerr
            << "cumetalc failed: stock Clang with CUDA support was not found; "
               "set CUMETAL_CLANG\n";
        return 1;
      }
      if (!cumetal::ir::llvm_frontend_available()) {
        std::cerr << "cumetalc failed: CuMetal was built without LLVM IRReader "
                     "support\n";
        return 1;
      }
      ...
      std::string command = quote_shell(clang.string()) +
      ...
      if (!ptx_feature.empty()) {
        command +=
            "-Xclang -target-feature -Xclang " + quote_shell(ptx_feature) + " ";
      }
      ...
      if (!clang_result.started || clang_result.exit_code != 0) {
        if (!clang_result.output.empty())
          std::cerr << clang_result.output;
      ...
      if (!opt_result.started || opt_result.exit_code != 0) {
        if (!opt_result.output.empty())
          std::cerr << opt_result.output;
      ...
      if (!command_exists("xcrun")) {
        std::cerr << "cumetalc failed: xcrun is required for .cu frontend "
                     "compilation\n";
        return 1;
      }
      if (!xcrun_tool_exists("clang++")) {
        std::cerr << "cumetalc failed: xcrun clang++ not available for .cu "
      ...
      const std::string command =
          ...
          "-D__global__= -D__host__= -D__device__= -D__shared__= "
          "-D__constant__= "
          "-D__managed__= " +
          ((std::filesystem::exists(runtime_api_dir) &&
            std::filesystem::is_directory(runtime_api_dir))
  ...
  if (!abi_sidecar.empty()) {
    const std::filesystem::path abi_path =
        options.output.string() + ".cumetal-abi";
    const std::vector<std::uint8_t> abi_bytes(abi_sidecar.begin(),
                                              abi_sidecar.end());
