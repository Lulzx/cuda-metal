...
- The typed shared-IR migration gate is not closed. With CUDA Clang 21-23, the
  reviewed 27-file production-metallib matrix is 27/27 for direct `.cu` and PTX
  through `cumetal-ir`, while the legacy PTX backend is 26/27 because it rejects
...
- The exact 25-project in-tree corpus now passes 25/25 through typed PTX and
  25/25 through direct native AOT on Apple M4 Pro with workload
