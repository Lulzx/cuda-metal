...
if [[ -z "${FAIL_REASON}" && "${NGL}" -gt 0 ]]; then
    if ! LC_ALL=C grep -aqE \
        'CUMETAL_PROVENANCE .*source=(generic_ptx|specialized_msl|metallib) provenance=(generic_ptx_lowering|generic_ptx_lowering_fp64_emulated|library_substitution|workload_specialization|precompiled_metallib) semantic_quality=(exact|reduced_precision_fp64|tolerance_bounded|semantic_emulation|performance_degraded) device=apple_gpu .*launch_success=true' \
        "${OUTPUT_FILE}"; then
