...
`--cuda-include`, and `--cuda-arch` are available for project builds. Jump tables are
disabled because CuMetal currently lowers structured PTX branches rather than
`brx.idx` target tables.
