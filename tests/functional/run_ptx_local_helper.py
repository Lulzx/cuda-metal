#!/usr/bin/env python3
"""Numerical Apple-GPU regression for generic-to-local helper pointer recovery."""
import ctypes as c
import os
...
PTX = (Path(__file__).parent / 'reference/ptx_local_helper.ptx').read_text()
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='cumetal-local-helper-') as work:
            ptx, msl = Path(work) / 'test.ptx', Path(work) / 'test.metal'
            ptx.write_text(PTX)
            subprocess.run([str(build / 'cumetalc'), str(ptx), '--backend=cumetal-ir',
                            '--ptx-strict', '--entry', 'local_helper_probe', '--emit=msl', '-o', str(msl)], check=True)
            api('cuModuleLoad', [c.POINTER(ptr), c.c_char_p], c.byref(module), os.fsencode(msl))
            api('cuModuleGetFunction', [c.POINTER(ptr), ptr, c.c_char_p], c.byref(function), module, b'local_helper_probe')
            ...
            print('NUMERICAL_PASS local_helper_probe: 12 runtime inputs; local helper identity, guards')
    finally:
