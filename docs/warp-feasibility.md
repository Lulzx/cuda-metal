...
- Warp checkout: `../warp-cumetal`, produced by `scripts/build_warp_cumetal.sh`
...
- The checkout is `v1.12.0` plus `scripts/warp-patches/`; nothing in this audit
  depends on those two patches, which only make the build reach CuMetal.
...
Reproduce all of this from a clean machine with:

```bash
scripts/build_warp_cumetal.sh
```

It clones NVIDIA/warp at `v1.12.0`, applies `scripts/warp-patches/`, generates
the CuMetal CUDA toolkit shim, and compiles each of the 11 `.cu` files through
it, printing the per-file result. Nothing is pushed anywhere and no fork is
required; see `scripts/warp-patches/README.md`.
...
`nvcc` shim with Warp's real release flags stands at **11 of 11 compiling** as
of 2026-09-05. This is a compile result: it says the CUDA these files are
written in reaches AIR, not that `libwarp` links or that the algorithms produce
Warp's answers on a GPU.
...
acceptance of `-gencode=…`/`-t0`. On the Warp side (`scripts/warp-patches/`,
applied to a pinned clone by `scripts/build_warp_cumetal.sh`):
...
| ~~`reduce.cu`, `runlength_encode.cu`~~ | **Compile (2026-09-05).** The CUB shim spelled these headers `.h`; CUB spells the device-wide ones `.cuh`, which is what Warp includes. Forwarding headers added. | CuMetal (landed) |
| ~~`sort.cu`, `sparse.cu`~~ | **Compile (2026-09-05).** Needed `cub::DoubleBuffer` (`cub/util_type.h`) and `DoubleBuffer` overloads of `DeviceRadixSort::Sort{Keys,Pairs}`, plus a `DeviceSegmentedRadixSort` the shim did not have at all. CuMetal's sort is host-backed and sorts in place, so the selector it hands back is the one it was given -- code that reads `Current()`, as both files do, is unaffected. | CuMetal (landed) |
| ~~`bvh.cu`~~ | **Compiles (2026-09-05).** `cub::BlockReduce` was a host-only sequential fallback: its methods were `__host__`, so a kernel could not call them, and its `TempStorage` was a `T` array, which is illegal in `__shared__` once `T` has a user-provided default constructor -- `wp::vec3` does. It is now a real cooperative tree reduction over threadgroup memory in device code, with the host fallback kept behind `__CUDA_ARCH__`, over uninitialized storage. `functional_cuda_projects_cub_block_reduce` runs it on the GPU. | CuMetal (landed) |
...
Two things the 11/11 result does *not* cover, both worth knowing before Phase 4
is called done. First, `cub::Device*` in CuMetal is host-backed: the algorithm
runs on the CPU over unified memory after synchronizing the stream, so `sort.cu`
and `sparse.cu` will produce correct answers but not device-parallel ones, and
Warp's sorting throughput on Metal is currently a CPU number. Second,
`bvh.cu:480` calls `BlockReduce::Reduce` from inside `if (tid < num_items)`.
Every CUB block primitive contains barriers, so on the tail block that is a
barrier in divergent control flow -- undefined in CUDA, tolerated by NVIDIA
hardware, and a plausible hang on Metal. Nothing here has run `compute_total_bounds`
on a GPU yet; that call site is the first thing to check when it does.
...
```bash
WARP=../warp-cumetal/warp/native   # scripts/build_warp_cumetal.sh --clone-only
