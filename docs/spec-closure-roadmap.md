...
| P2 | Runtime semantic subsets | Expand graph node/cross-stream/allocator semantics; direct PTX texture/surface instructions; irregular cooperative-group masks; device-string/dynamic-width `printf`; and observable `ieee64` exception status. Preserve the now-tested nested/invalid/overflow dynamic-launch queue and wide scalar `printf` behavior plus allocation/pointer, stream/event, launch ABI, provenance, and per-thread error invariants. | Focused positive and negative tests plus numerical Apple-GPU execution and unmodified workloads where available; every unsupported branch fails explicitly. |
