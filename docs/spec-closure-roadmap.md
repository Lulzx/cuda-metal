...
| P1 | Canonical compiler path | Expand residual generic-pointer/device-call/CFG combinations beyond the now-matched 24-file compile corpus and broaden native-AOT metadata for writable/zero-initialized CUDA globals. Preserve cache identity/corruption rejection throughout the migration. | `conformance_compiler_backend_matrix_versions` plus numerical Apple-GPU tests show typed IR at least matches the legacy pass count across CUDA Clang 21-23; `functional_cumetalc_link_executable` proves an embedded native module with no `__cudaRegister*` dependency, no first-launch PTX JIT, and no registration-JIT cache; `unit_module_cache` remains green. Only then change remaining backend defaults. |
