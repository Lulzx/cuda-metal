...
id<MTLComputePipelineState> load_pipeline_locked(BackendState& backend,
    ...
    @autoreleasepool {
        ...
        for (id<MTLBinding> binding in [reflection bindings]) {
            ...
            if ([binding index] == cumetal::ptx::kTrapStatusBindingIndex &&
                [[binding name] isEqualToString:@"cm_trap_status"]) uses_trap_status = true;
...
cudaError_t launch_kernel(const std::string& metallib_path,
    ...
    if (needs_trap_status) {
        for (std::size_t i = 0; i < args.size(); ++i) {
            ...
            if (binding == cumetal::ptx::kTrapStatusBindingIndex) {
    ...
    @autoreleasepool {
        ...
        if (trap_status) {
            ...
            [encoder setBuffer:buffer->handle() offset:0 atIndex:cumetal::ptx::kTrapStatusBindingIndex];
        }
