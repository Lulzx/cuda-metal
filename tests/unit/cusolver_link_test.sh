...
for lib in libcusparse.dylib libcusolver.dylib libcublasLt.dylib libcudnn.dylib libnvidia-ml.dylib libnccl.dylib libnvrtc.dylib; do
    target="$LIB_DIR/$lib"
    if [ ! -e "$target" ]; then
...
echo "PASS: all library symlinks present"
