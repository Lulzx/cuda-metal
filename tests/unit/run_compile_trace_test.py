...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='cumetal-trace-host-') as work:
        ...
        assert begin_stages(records) == [
            'ptx_to_msl_total', 'ptx_import', 'metal_legalization', 'msl_generation']
        ...
        assert begin_stages(parse_spans(traced_failure.stderr)) == [
            'ptx_to_msl_total', 'ptx_import'], traced_failure.stderr
